...
import {
  LineChart, CartesianGrid, XAxis, YAxis, Tooltip, Line, ResponsiveContainer,
...
const AdminDashboard = () => {
  ...
  const fetchRevenueData = () => {
    const token = localStorage.getItem('token');
    let url = `http://localhost:8080/api/admin/dashboard/revenue-stats?type=${revenueChartType}`;
    if (appliedDateRange[0] && appliedDateRange[1]) {
      const start = appliedDateRange[0].toISOString().split('T')[0];
      const end = appliedDateRange[1].toISOString().split('T')[0];
      url += `&startDate=${start}&endDate=${end}`;
    }
    console.log('Fetching revenue data with URL:', url);
    axios.get(url, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then(res => {
      ...
      .catch(err => {
        ...
        alert('Lỗi khi lấy dữ liệu doanh thu. Vui lòng thử lại.');
      });
  };

  useEffect(() => {
    fetchRevenueData();
  }, [revenueChartType, appliedDateRange]);
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex items-center gap-4">
              <DatePicker
                ...
                endDate={tempEndDate}
                onChange={(update) => {
                  console.log('DatePicker changed:', update);
              ...
              <button
                ...
                className="bg-emerald-500 text-white px-4 py-2 rounded-lg text-sm hover:bg-emerald-600 disabled:bg-emerald-300 disabled:cursor-not-allowed transition duration-200"
              >
                Lọc
              </button>
              <select
